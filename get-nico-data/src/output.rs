    ...
    for packet in receiver.iter() {
        for video in packet.videos {
            bincode::serialize_into(&mut writer, &NewVideoInfo {
                ...
                thumbnail_url: video.thumbnail_url.unwrap(),
                start_time: video.start_time.unwrap(),
                last_res_body: video.last_res_body.unwrap(),
                comment_counter: video.comment_counter.unwrap(),
                last_comment_time: video.last_comment_time.unwrap(),
                category_tags: video.category_tags.unwrap(),
                tags: video.tags.unwrap(),
                genre: video.genre.unwrap(),
                genre_keyword: video.genre_keyword.unwrap(),
            }).unwrap();
        }
    }
}